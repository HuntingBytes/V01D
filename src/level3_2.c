...
void mainLevel3_2()
{
    ...
    Rectangle frameRecstopped_right = {0.0f, 0.0f, (float) spritestopped_right.width / 4, (float) spritestopped_right.height};
    Rectangle frameRecstopped_left = {0.0f, 0.0f, (float) spritestopped_left.width / 4, (float) spritestopped_left.height};
    ...
    int life_of_broken_rec1=10;
    int life_of_broken_rec2=5;
    ...
    //UPDATES------------------------------------------------------
        ...
        if(CheckCollisionRecs(bullet_position,limitsright_forbullet)||CheckCollisionRecs(bullet_position,limitsleft_forbullet)||CheckCollisionRecs(bullet_position,broken_rec1)||CheckCollisionRecs(bullet_position,broken_rec2)){
        ...
        if(cangravity==false){
            ...
            if(controljump>maxpixelsjump){ // this if is for the computer know what the time to start the applying the gravity
                cangravity=1;
                controljump=false;
            }
                ...
                if (IsKeyDown(KEY_LEFT)){// while player jump, he can go to left
        ...
        //IF OF GRAVITY
        ...
        else if(!(CheckCollisionRecs(how_far_gravity_is_applied, limitsdown)  || CheckCollisionRecs(how_far_gravity_is_applied, rec2) || CheckCollisionRecs(how_far_gravity_is_applied, rec3)  || CheckCollisionRecs(how_far_gravity_is_applied, stair)) && cangravity == 1){
            ...
            if (IsKeyDown(KEY_LEFT)){
                ...
                if((trackposition.x<624 && trackposition.x>482) && (trackposition.y<216 && trackposition.y>118)){//bug of rec2 collision
                    trackposition.x=624;

                }
                if((trackposition.x<690 && trackposition.x>628) && (trackposition.y<214 && trackposition.y>128)){//bug rec 3 collision
                    trackposition.x=690;
                ...
                immediately_previous_position.x = trackposition.x + 2.0f; //updating previous position x
            ...
            if (IsKeyDown(KEY_RIGHT)){
                ...
                if((trackposition.x<644 && trackposition.x>628) && (trackposition.y<204 && trackposition.y>126)){//bug rec 3 collision
                    trackposition.x=628;
                ...
                immediately_previous_position.x= trackposition.x - 2.0f;//update previous position x
        ...
        else if(CheckCollisionRecs(trackposition,limitsdown)||CheckCollisionRecs(trackposition,rec2)||CheckCollisionRecs(trackposition,rec3)) {
        ...
        //IF OF COMPUTE THE MOVEMENTS IN STAIR
            ...
            if (IsKeyDown(KEY_UP)) {
                ...
                immediately_previous_position.y= trackposition.y + 2.0f;//saving the before the position of trackposition
            ...
            if (IsKeyDown(KEY_LEFT)){
                ...
                if((trackposition.x<623 && trackposition.x>618) && (trackposition.y<160 && trackposition.y>72)){//bug of rec2 collision
                    trackposition.x=623;
        ...
        else if (CheckCollisionRecs(trackposition,AllowRightLeftinBottom)||CheckCollisionRecs(trackposition,AllowRightLeftintop)){
            if (IsKeyDown(KEY_RIGHT)){
        ...
        if(life_of_broken_rec1>0 && life_of_broken_rec2==5){
        ...
        if(CheckCollisionRecs(trackposition,lever)){
            DrawText("Type The Key!", 530, 310, 20,WHITE);
            ...
            DrawText(TextFormat("INPUT CHARS: %i/%i", letterCount, MAX_INPUT_CHARS), 530, 340, 20,WHITE);
            ...
            if (letterCount < MAX_INPUT_CHARS)
            {
                ...
                if (((FramesCounter_Enigma)%2) == 0) DrawText("_", (int)textBox.x + 8 + (int)MeasureText(name, 40), (int)textBox.y + 12, 40, MAROON);
            }
            else DrawText("Press BACKSPACE to delete chars...", 530, 360, 20, WHITE);
